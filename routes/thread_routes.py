...
@thread_routes.post("/chat/agentic")
@handle_exceptions
def chat_agentic():
    """
    Agentic chat endpoint using LangGraph multi-agent system
    """
    validated_request_data = chat_messages_schema.load(request.get_json())
    messages = thread_controller.process_chat_message_agentic(validated_request_data.get("messages"))
    return Response.make(messages, Response.HTTP_SUCCESS)