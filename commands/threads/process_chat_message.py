...
class ProcessChatMessageCommand(ReadCommand):
    ...
    def execute(self) -> None:
        ...
        if response.choices[0].finish_reason == "tool_calls":
            ...
            # Make a follow-up call to get natural language analysis
            try:
                follow_up_kwargs = {
                    "messages": self.prepare_chat_messages(),
                    # Don't include tools in follow-up to force natural language response
                }
                
                follow_up_response = self.llm_session.chat(**follow_up_kwargs)
                
                # Add the natural language analysis response
                analysis_message = self.format_message(
                    role="assistant",
                ...
                logger.error(f"Failed to generate follow-up analysis: {e}")
                # If follow-up fails, continue with just the tool results
